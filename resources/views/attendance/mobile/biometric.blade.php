...
<style>
/* ==========================================================================
   ARISE ERP - NATIVE MOBILE BIOMETRIC & DAILY ATTENDANCE INTERFACE
   ========================================================================== */
...
/* 3. Class / Role Selector & Bulk Time Toolbar */
.att-mob-filter-card {
    background: #ffffff;
...
/* Card Time Input Grid */
.att-mob-card-time-row {
    display: grid;
...
<div class="att-mob-hero">
    <div class="att-mob-hero-top">
        <div class="att-mob-hero-title">
            <i class="fa fa-calendar-check-o text-primary"></i> Biometric Attendance
        </div>
    ...
    <div class="att-mob-metrics">
        ...
        <div class="att-mob-metric-box">
            <span class="att-mob-metric-tag">In</span>
...
<div class="att-mob-filter-card">
    ...
    {{-- Bulk Check-In / Out Time Toolbar --}}
    <div class="att-mob-bulk-time-bar">
        <span class="att-bulk-time-label"><i class="fa fa-clock-o"></i> Time:</span>
        <input type="time" id="mobBulkInTime" class="att-bulk-time-input" placeholder="In" title="Bulk Check-in Time" @if($holidayLock) disabled @endif>
        <input type="time" id="mobBulkOutTime" class="att-bulk-time-input" placeholder="Out" title="Bulk Check-out Time" @if($holidayLock) disabled @endif>
        <button type="button" class="btn-att-bulk-set-time" id="btnMobApplyBulkTime" @if($holidayLock) disabled @endif>
            Set Time
        </button>
    </div>
</div>
...
@if($totalCount > 0)
    <div class="att-mob-bulk-bar">
        <button type="button" class="btn-mob-quick btn-mob-quick-in" id="btnMobAllPresent" @if($holidayLock) disabled @endif>
            <i class="fa fa-check-circle"></i> All In
        </button>
    ...
    <div class="att-mob-cards-feed" id="attendanceCardsFeed">
        @forelse($activeItems as $i => $rowItem)
            @php
                ...
                if (empty($statusVal) && !empty($inTime)) $statusVal = 'in';
                ...
                $inTimeVal = $inTime ? date('H:i', strtotime($inTime)) : '';
                $outTimeVal = $outTime ? date('H:i', strtotime($outTime)) : '';
            @endphp
            ...
            <div class="att-mob-card {{ $cardClass }}"
                ...
                {{-- Biometric Check-In & Check-Out Time Row --}}
                <div class="att-mob-card-time-row">
                    <div class="att-mob-time-box">
                        <label><i class="fa fa-sign-in text-success mr-1"></i>In:</label>
                        <input type="time" class="att-mob-time-input card-in-time"
                            name="rows[{{ $i }}][in_time]"
                            value="{{ $inTimeVal }}"
                            @if($holidayLock) disabled @endif>
                    </div>
                    <div class="att-mob-time-box">
                        <label><i class="fa fa-sign-out text-danger mr-1"></i>Out:</label>
                        <input type="time" class="att-mob-time-input card-out-time"
        ...
        @empty
            <div class="att-mob-empty">
                ...
                <div class="att-mob-empty-title">
                    @if(!$isStaffTab)
                        {{ request('class_type_id') ? 'No students found in this class.' : 'Select a class to mark biometric attendance.' }}
                    @else
...
$(document).ready(function() {
    ...
    // 1. Snapshot original data for modification tracking
    $('.att-mob-card').each(function() {
        const $card = $(this);
        const origSt = $card.find('.hidden-status-input').val() || '';
        const origIn = $card.find('.card-in-time').val() || '';
        const origOut = $card.find('.card-out-time').val() || '';
        $card.data('orig', { status: origSt, inTime: origIn, outTime: origOut });
    });
    ...
    function formatStatusInfo(st) {
        switch(st) {
            case 'in':
            case 'present':
                return { label: 'In / Present', badgeClass: 'badge-in', cardClass: 'card-status-in' };
            case 'absent':
    ...
    function recalcCounters() {
        ...
        $('.att-mob-card:visible').each(function() {
            ...
            const inT = $c.find('.card-in-time').val() || '';
            const outT = $c.find('.card-out-time').val() || '';
            ...
            const orig = $c.data('orig') || {};
            if (st !== (orig.status || '') || inT !== (orig.inTime || '') || outT !== (orig.outTime || '')) {
                modified++;
            }
        });
    ...
    $(document).on('change', '.card-in-time, .card-out-time', function() {
        const $card = $(this).closest('.att-mob-card');
        const inT = $card.find('.card-in-time').val();
        const currentSt = $card.find('.hidden-status-input').val();
        if (inT && (!currentSt || currentSt === 'absent')) {
            setCardStatus($card, 'in');
        } else {
            recalcCounters();
        }
    });
    ...
    $('#btnMobAllReset').on('click', function() {
        showMobileConfirm(
            'Reset Attendance',
            'Are you sure you want to reset attendance & time for all visible records?',
            function() {
                $('.att-mob-card:visible').each(function() {
                    const $c = $(this);
                    $c.find('.card-in-time').val('');
                    $c.find('.card-out-time').val('');
                    setCardStatus($c, '');
                });
    ...
    // Bulk Time Apply
    $('#btnMobApplyBulkTime').on('click', function() {
        if (isHolidayLocked) return;
        const inT = $('#mobBulkInTime').val();
        const outT = $('#mobBulkOutTime').val();

        if (!inT && !outT) {
            showMobileToast('Please select In Time or Out Time first.', 'warning');
            return;
        }

        $('.att-mob-card:visible').each(function() {
            const $c = $(this);
            if (inT) $c.find('.card-in-time').val(inT);
            if (outT) $c.find('.card-out-time').val(outT);
            const curSt = $c.find('.hidden-status-input').val();
            if (!curSt || curSt === 'absent') {
                setCardStatus($c, 'in');
            } else {
                recalcCounters();
            }
        });
        showMobileToast('Bulk time applied to visible records.', 'success');
    });
    ...
    $('#mobAttendanceForm').on('submit', function(e) {
        ...
        $('.att-mob-card').each(function() {
            ...
            const inTime = $c.find('.card-in-time').val() || null;
            const outTime = $c.find('.card-out-time').val() || null;

            const isChanged = currentStatus !== (orig.status || '') ||
                              inTime !== (orig.inTime || null) ||
                              outTime !== (orig.outTime || null);
