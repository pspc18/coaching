...
@php
    ...
    foreach ($activeItems as $item) {
        ...
        $st = $m->status ?? '';
        if ($st === 'in' || $st === 'present' || (!empty($m->in_time) && empty($st))) {
            $markedInCount++;
        } elseif ($st === 'absent') {
            $markedAbsentCount++;
        } elseif ($st === 'halfday') {
...
.att-badge-pill.badge-time { background: #ffedd5; color: #9a3412; border: 1px solid #fed7aa; }
...
<div class="content-wrapper att-page-wrapper">
            ...
            <div class="att-layout">
                ...
                <div class="att-hero">
                    <div class="att-hero-left">
                        ...
                        <h1 class="att-hero-title">Biometric &amp; Daily Attendance Marking</h1>
                        <p class="att-hero-subtitle">Mark daily attendance, check-in &amp; check-out timestamps, and bulk status updates for students and staff</p>
                    </div>
                    ...
                    <div class="att-hero-right">
                        ...
                        <div class="att-hero-stats">
                            ...
                            <span class="stat-pill pill-green" title="Checked In / Present">
                                <i class="fa fa-check-circle"></i> In: <b id="stat-in">{{ $markedInCount }}</b>
                            </span>
                ...
                <form method="post" action="{{ $isStaffTab ? $staffSaveUrl : $studentsSaveUrl }}" id="attendance-main-form" class="d-flex flex-column flex-grow-1 min-h-0 m-0">
                    ...
                    <div class="att-action-bar">
                        ...
                        <div class="att-action-right">
                            {{-- Bulk Check-in & Check-out Time --}}
                            <div class="att-bulk-group">
                                <span class="att-bulk-label">Bulk Time:</span>
                                <input type="time" id="bulkInTime" class="att-time-input-sm" step="60" title="Bulk In Time" @if($holidayLock) disabled @endif>
                                <input type="time" id="bulkOutTime" class="att-time-input-sm" step="60" title="Bulk Out Time" @if($holidayLock) disabled @endif>
                                <button type="button" class="btn-att-action" id="btn-apply-bulk-time" title="Apply time to selected rows" @if($holidayLock) disabled @endif>
                            ...
                            <div class="att-bulk-group">
                                ...
                                <select id="bulkStatusSelect" class="att-select" style="min-width: 105px; height: 24px; padding: 1px 4px;" @if($holidayLock) disabled @endif>
                            ...
                            <div class="d-none d-lg-inline-flex align-items-center" style="gap: 4px;">
                                <button type="button" class="btn-quick-status btn-quick-in" id="btn-quick-all-in" title="Mark selected Present / In" @if($holidayLock) disabled @endif>
                                    <i class="fa fa-check"></i> All In
                                </button>
                            ...
                            <table class="att-grid-table" id="attendanceGridTable">
                                <thead>
                                    <tr>
                                        ...
                                        <th style="width: 130px;" class="text-center">Unique ID</th>
                                        <th style="min-width: 180px;">Name</th>
                                        <th style="width: 125px;" class="text-center">Check In</th>
                                        <th style="width: 125px;" class="text-center">Check Out</th>
                                ...
                                <tbody>
                                    @forelse($activeItems as $i => $rowItem)
                                        ...
                                        <tr class="att-record-row {{ $rowStatusClass }}"
                                            ...
                                            {{-- Check In Time --}}
                                            <td class="text-center">
                                                <input type="time" class="att-row-time row-in-time"
                                                    name="rows[{{ $i }}][in_time]"
                                                    value="{{ $inTime ? date('H:i', strtotime($inTime)) : '' }}"
                                                    step="60"
                                            ...
                                            {{-- Check Out Time --}}
                                            <td class="text-center">
                                                <input type="time" class="att-row-time row-out-time"
                                                    name="rows[{{ $i }}][out_time]"
                                                    value="{{ $outTime ? date('H:i', strtotime($outTime)) : '' }}"
                                                    step="60"
                                                    @if($holidayLock) disabled @endif>
                                            </td>
    ...
    $(document).ready(function() {
        ...
        // Helper: Convert status value to human readable format
        ...
        // Helper: Refresh visual badge and row background based on row status
        function updateRowAppearance($row) {
            const $statusSelect = $row.find('.row-status-select');
            const $badge = $row.find('.row-status-badge');
            const inTime = ($row.find('.row-in-time').val() || '').trim();
            const outTime = ($row.find('.row-out-time').val() || '').trim();
            ...
            const allBadgeClasses = 'badge-in badge-present badge-absent badge-halfday badge-holiday badge-leave badge-time badge-none';
            ...
            if (statusVal) {
                ...
                $badge.addClass('badge-time').text('Time Set');
            } else {
                $statusSelect.addClass('status-none');
        ...
        // Recalculate summary stats on the fly
        function recalcStats() {
            ...
            $('.att-record-row:visible').each(function() {
                ...
                const inT = ($r.find('.row-in-time').val() || '').trim();
                const outT = ($r.find('.row-out-time').val() || '').trim();

                if (st === 'in' || st === 'present' || (inT && !st)) {
                    markedIn++;
                } else if (st === 'absent') {
                    markedAbsent++;
                } else if (st === 'halfday') {
                ...
                // Check modified against snapshot
                const orig = $r.data('att-orig') || {};
                const isMod = inT.substring(0, 5) !== (orig.inTime || '')
                    || outT.substring(0, 5) !== (orig.outTime || '')
                    || st !== (orig.status || '');
                if (isMod) {
                    modifiedCount++;
                }
        ...
        // Snapshot original data on page load for performance
        $('.att-record-row').each(function() {
            const $r = $(this);
            const origIn = ($r.find('.row-in-time').val() || '').trim().substring(0, 5);
            const origOut = ($r.find('.row-out-time').val() || '').trim().substring(0, 5);
        ...
        // Dropdown filter change (Class / Role)
        ...
        $(document).on('change blur', '.row-in-time, .row-out-time', function() {
            updateRowAppearance($(this).closest('tr'));
        });
        ...
        $(document).on('click', '.btn-row-reset', function() {
            ...
            $r.find('.row-in-time').val('');
            $r.find('.row-out-time').val('');
        ...
        $('#btn-apply-bulk-time').on('click', function() {
            if (isHolidayLocked) return;
            const inTime = $('#bulkInTime').val();
            const outTime = $('#bulkOutTime').val();

            if (!inTime && !outTime) {
                alert('Please specify Check In or Check Out time to apply.');
                return;
            }
            ...
            $('.att-record-row:visible').each(function() {
                const $r = $(this);
                if ($r.find('.row-check').is(':checked')) {
                    if (inTime) $r.find('.row-in-time').val(inTime);
                    if (outTime) $r.find('.row-out-time').val(outTime);
                    updateRowAppearance($r);
                }
            });
        });
        ...
        $('#btn-quick-reset-all').on('click', function() {
            if (isHolidayLocked) return;
            if (!confirm('Reset time and status for all selected rows?')) return;
            $('.att-record-row:visible').each(function() {
                const $r = $(this);
                if ($r.find('.row-check').is(':checked')) {
                    $r.find('.row-in-time').val('');
        ...
        // Gathers only changed/selected records and packages them as JSON
        $('#attendance-main-form').on('submit', function(e) {
            ...
            $('.att-record-row').each(function() {
                ...
                const orig = $r.data('att-orig') || {};
                const currentIn = ($r.find('.row-in-time').val() || '').trim().substring(0, 5);
                const currentOut = ($r.find('.row-out-time').val() || '').trim().substring(0, 5);
                const currentStatus = $r.find('.row-status-select').val() || '';

                const isChanged = currentIn !== (orig.inTime || '')
                    || currentOut !== (orig.outTime || '')
                    || currentStatus !== (orig.status || '');
            ...
            // Prevent submission of thousands of raw POST inputs to bypass PHP max_input_vars limit
            const $form = $(this);
            ...
            // Button visual loading indicator
