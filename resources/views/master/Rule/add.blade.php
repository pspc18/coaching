    ...
    <div class="container-fluid p-2">
        <div class="admission-page-layout">
            ...
            <div class="admission-hero">
                <div>
                    <span class="admission-kicker"><i class="fa fa-gavel mr-1"></i> ARIS MASTER MANAGEMENT</span>
                    <h1 class="admission-title">Rules & Regulations Master</h1>
                    <p class="admission-subtitle">Define institutional guidelines, student conduct codes, and staff policies</p>
                </div>
                <div class="admission-hero-actions">
                    <button type="button" class="dash-btn dash-btn-light" data-toggle="modal" data-target="#addRuleModal">
                        <i class="fa fa-plus-circle mr-1 text-primary"></i> Add New Rule
                    </button>
                </div>
            </div>
            ...
            <div class="dash-card admission-table-card">
                <div class="dash-card-header d-flex align-items-center justify-content-between">
                    <div class="d-flex align-items-center" style="gap: 8px;">
                        ...
                        <h2 class="dash-card-title">Rules & Regulations Directory</h2>
                    </div>
                    <div class="d-flex align-items-center" style="gap: 8px;">
                ...
                <div class="table-scroll-container">
                    <div id="tableLoadingOverlay" class="table-loading-overlay" style="display: none;">
                        <div class="loading-badge">
                            <i class="fa fa-spinner fa-spin text-info"></i> Updating rules list...
                        </div>
                    </div>

                    <table class="dash-table" id="rulesTable">
                        <thead>
                            <tr class="header-titles-row">
                                <th style="width: 65px;" class="text-center">#ID</th>
                                <th style="min-width: 200px;">Rule Title</th>
                                <th style="width: 170px;" class="text-center">Applicable Role</th>
                            ...
                            <tr class="excel-filter-row">
                                ...
                                <th>
                                    <input type="text" class="excel-col-filter" id="col_filter_name" placeholder="Filter title..." value="{{ $searchName ?? '' }}" autocomplete="off">
                                </th>
                                <th>
                                    <select class="excel-col-filter" id="col_filter_role">
                                        <option value="all">All Roles</option>
                                        @foreach($allRoles as $roleItem)
                                            <option value="{{ $roleItem->id }}" {{ ($roleFilter ?? '') == (string)$roleItem->id ? 'selected' : '' }}>
                                                {{ $roleItem->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <th>
                                    <input type="text" class="excel-col-filter" id="col_filter_desc" placeholder="Filter description details..." value="{{ $searchDesc ?? '' }}" autocomplete="off">
                                </th>
                        ...
                        <tbody id="rules_tbody">
                            @include('master.Rule.rule_rows', ['rows' => $rows])
                        </tbody>
                    </table>
                </div>
    ...
    <div class="modal-dialog modal-md modal-dialog-centered" role="document">
        <div class="modal-content" style="border-radius: 4px; overflow: hidden; border: 1px solid #002C54; box-shadow: 0 10px 25px rgba(0,0,0,.2);">
            <div class="modal-header" style="background: linear-gradient(135deg, #002C54 0%, #0f3460 100%); color: #fff; padding: 10px 14px;">
                <h5 class="modal-title" id="addRuleModalLabel" style="font-size: 13px; font-weight: 700;">
                    <i class="fa fa-plus-circle mr-1"></i> Add Institutional Rule
                </h5>
                <button type="button" class="close text-white" data-dismiss="modal" aria-label="Close" style="opacity: .9; outline: none;">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form id="addRuleForm" action="{{ url('rules_add') }}" method="post">
                @csrf
                <div class="modal-body" style="padding: 14px; font-size: 12px; background: #fff;">
                    <div class="form-group mb-2">
                        <label class="font-weight-bold text-dark mb-1" style="font-size: 11.5px;">Rule Title <span class="text-danger">*</span></label>
                        <input type="text" name="name" class="form-control form-control-sm" placeholder="e.g. Attendance Requirement, Uniform Policy" required autocomplete="off">
                    </div>
                    <div class="form-group mb-2">
                        <label class="font-weight-bold text-dark mb-1" style="font-size: 11.5px;">Target Applicable Role <span class="text-danger">*</span></label>
                    ...
                    <div class="form-group mb-0">
                        <label class="font-weight-bold text-dark mb-1" style="font-size: 11.5px;">Description / Policy Details</label>
                        <textarea name="description" class="form-control form-control-sm" rows="4" placeholder="Enter complete policy text, rules, and guidelines..."></textarea>
                    </div>
                </div>
                <div class="modal-footer" style="padding: 8px 14px; background: #f8fafc; border-top: 1px solid #e2e8f0; display: flex; justify-content: flex-end; gap: 6px;">
                    ...
                    <button type="submit" class="btn btn-sm btn-primary" id="btnSubmitAddRule" style="font-size: 11px; padding: 4px 12px; background: #002C54; border-color: #002C54;">
                        <i class="fa fa-save mr-1"></i> Save Rule
                    </button>
                </div>
            </form>
        </div>
...
<div class="modal fade" id="editRuleModal" tabindex="-1" role="dialog" aria-labelledby="editRuleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-md modal-dialog-centered" role="document">
        <div class="modal-content" style="border-radius: 4px; overflow: hidden; border: 1px solid #002C54; box-shadow: 0 10px 25px rgba(0,0,0,.2);">
            <div class="modal-header" style="background: linear-gradient(135deg, #002C54 0%, #0f3460 100%); color: #fff; padding: 10px 14px;">
                <h5 class="modal-title" id="editRuleModalLabel" style="font-size: 13px; font-weight: 700;">
                    <i class="fa fa-edit mr-1"></i> Edit Institutional Rule
                </h5>
                <button type="button" class="close text-white" data-dismiss="modal" aria-label="Close" style="opacity: .9; outline: none;">
                    <span aria-hidden="true">&times;</span>
                </button>
            ...
            <form id="editRuleForm" method="post">
                @csrf
                <input type="hidden" id="edit_rule_id" name="id" value="">
                <div class="modal-body" style="padding: 14px; font-size: 12px; background: #fff;">
                    <div class="form-group mb-2">
                        <label class="font-weight-bold text-dark mb-1" style="font-size: 11.5px;">Rule Title <span class="text-danger">*</span></label>
                        <input type="text" id="edit_rule_name" name="name" class="form-control form-control-sm" required autocomplete="off">
                    </div>
                    <div class="form-group mb-2">
                        <label class="font-weight-bold text-dark mb-1" style="font-size: 11.5px;">Target Applicable Role <span class="text-danger">*</span></label>
                        <select id="edit_rule_role_id" name="role_id" class="form-control form-control-sm" required>
                            <option value="">-- Select Target Role --</option>
                            @foreach($allRoles as $roleOption)
                                <option value="{{ $roleOption->id }}">{{ $roleOption->name }}</option>
                            @endforeach
                    ...
                    <div class="form-group mb-0">
                        <label class="font-weight-bold text-dark mb-1" style="font-size: 11.5px;">Description / Policy Details</label>
                        <textarea id="edit_rule_description" name="description" class="form-control form-control-sm" rows="4" placeholder="Enter complete policy text, rules, and guidelines..."></textarea>
                    </div>
                </div>
                <div class="modal-footer" style="padding: 8px 14px; background: #f8fafc; border-top: 1px solid #e2e8f0; display: flex; justify-content: flex-end; gap: 6px;">
                    <button type="button" class="btn btn-sm btn-secondary" data-dismiss="modal" style="font-size: 11px; padding: 4px 10px;">Cancel</button>
                    <button type="submit" class="btn btn-sm btn-primary" id="btnSubmitEditRule" style="font-size: 11px; padding: 4px 12px; background: #002C54; border-color: #002C54;">
                        <i class="fa fa-check mr-1"></i> Update Rule
                    </button>
                </div>
            </form>
        </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const filterRole = document.getElementById('col_filter_role');
    const filterDesc = document.getElementById('col_filter_desc');
    ...
    function fetchRules(targetPage = 1) {
        ...
        const params = new URLSearchParams({
            ...
            role_id: filterRole ? filterRole.value : 'all',
            search_desc: filterDesc ? filterDesc.value.trim() : '',
        });

        fetch(`{{ url('rules_add') }}?${params.toString()}`, {
            headers: {
    ...
    [filterId, filterName, filterDesc].forEach(input => {
        if (input) {
    ...
    if (filterRole) {
        filterRole.addEventListener('change', () => fetchRules(1));
    }

    if (perPageSelect) {
        perPageSelect.addEventListener('change', () => fetchRules(1));
    }
    ...
    if (btnLast) {
        btnLast.addEventListener('click', () => {
            if (currentPage < totalPages) fetchRules(totalPages);
        });
    }
    ...
    document.addEventListener('click', function(e) {
        ...
        if (editBtn) {
            ...
            document.getElementById('edit_rule_role_id').value = roleId;
            document.getElementById('edit_rule_description').value = description;
            document.getElementById('editRuleForm').action = `{{ url('rules_edit') }}/${id}`;
        }
    });

    // Add Rule Form AJAX Submit
    const addRuleForm = document.getElementById('addRuleForm');
    if (addRuleForm) {
        addRuleForm.addEventListener('submit', function (e) {
            e.preventDefault();
            const submitBtn = document.getElementById('btnSubmitAddRule');
            if (submitBtn) {
            ...
            .then(data => {
                if (submitBtn) {
                    ...
                    submitBtn.innerHTML = '<i class="fa fa-save mr-1"></i> Save Rule';
                }
                if (data.status === 'success') {
                    $('#addRuleModal').modal('hide');
                    addRuleForm.reset();
                    if (typeof toastr !== 'undefined') {
                        toastr.success(data.message || 'Rule added successfully.');
                    } else {
                        alert(data.message || 'Rule added successfully.');
                    }
                    fetchRules(1);
                } else {
                    if (typeof toastr !== 'undefined') {
                        toastr.error(data.message || 'Failed to save rule.');
                    } else {
    ...
    if (editRuleForm) {
        editRuleForm.addEventListener('submit', function (e) {
            ...
            .then(data => {
                if (submitBtn) {
                    submitBtn.disabled = false;
                    submitBtn.innerHTML = '<i class="fa fa-check mr-1"></i> Update Rule';
                }
                if (data.status === 'success') {
                    $('#editRuleModal').modal('hide');
                    if (typeof toastr !== 'undefined') {
                        toastr.success(data.message || 'Rule updated successfully.');
                    } else {
                        alert(data.message || 'Rule updated successfully.');
                    }
                    fetchRules(currentPage);
                } else {
                    if (typeof toastr !== 'undefined') {
                        toastr.error(data.message || 'Failed to update rule.');
                    } else {
    ...
    // Delete Rule AJAX Form handling
    document.addEventListener('submit', function(e) {
        const delForm = e.target.closest('.js-delete-rule-form');
        if (delForm) {
            e.preventDefault();
            ...
            .then(data => {
                if (data.status === 'success') {
                    if (typeof toastr !== 'undefined') {
                        toastr.success(data.message || 'Rule deleted successfully.');
                    } else {
                        alert(data.message || 'Rule deleted successfully.');
                    }
                    fetchRules(currentPage);
                } else {
                    if (typeof toastr !== 'undefined') {
                        toastr.error(data.message || 'Failed to delete rule.');
                    } else {
