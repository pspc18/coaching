...
function imgBase64($url) {
    $parts = explode('/', $url);
    $parts[count($parts)-1] = rawurlencode(end($parts)); // encode filename only
    $url = implode('/', $parts);
    $data = @file_get_contents($url);
    return $data ? 'data:image/' . pathinfo($url, PATHINFO_EXTENSION) . ';base64,' . base64_encode($data) : null;
}

$logoBase64 = !empty($getSetting['left_logo'])
    ? imgBase64(rtrim(env('IMAGE_SHOW_PATH'), '/') . '/setting/left_logo/' . $getSetting['left_logo'])
    : null;

if (!$logoBase64) {
    $logoBase64 = imgBase64(rtrim(env('IMAGE_SHOW_PATH'), '/') . '/default/rukmanisoft_logo.png');
}
@endphp
...
<table style="border-bottom: 0px solid;width:100%;" >
	...
	<tr>
      <td class="border_none" rowspan='1' colspan="12"width='20%'>
          <img  rowspan="4" src="{{ $logoBase64 }}" style="width: 97px;">
