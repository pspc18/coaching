				...
				<div class="col-12 col-md-12">
					<div class="card card-outline card-orange">
						<div class="card-header bg-primary">
							...
							<div class="card-tools"> <a href="{{url('evente/certificate/add')}}" class="btn btn-primary  btn-sm {{ Helper::permissioncheck(5)->add ? '' : 'd-none' }}"><i class="fa fa-plus"></i>{{ __('common.Add') }} </a> 
                      ...
                      @if(!empty($data))
                        ...
                        <tr>
                                ...
                                <td>{{ $i++ }}</td>
                                    ...
                                    <td>{{ !empty($item['organized_date']) ? date('d-m-Y', strtotime($item['organized_date'])) : '' }}</td>
                                    <!--<td>{{ $item['rank'] ?? '' }}</td>-->
                                   
                                <td>
                                    
                                      <a href="{{url('evente/certificate/edit',$item->id)}}" title="Edit" class="{{ Helper::permissioncheck(5)->edit ? '' : 'd-none' }}"><i class="fa fa-edit text-primary"></i></a>
                                      <a href="javascript:;"  data-id='{{$item->id}}' data-bs-toggle="modal" data-bs-target="#Modal_id"  class="deleteData {{ Helper::permissioncheck(5)->delete ? '' : 'd-none' }}" title="Delete"><i class="fa fa-trash-o text-danger"></i></a>
                                      
                                     <a href="{{url('evente_print',$item->id)}}" target="blank" title="Print" class="{{ Helper::permissioncheck(5)->print ? '' : 'd-none' }}"><i class="fa fa-print text-success"></i></a>
